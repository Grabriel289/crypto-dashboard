      ...
      <div className="mb-6">
        ...
        <div className="space-y-2">
          {macro_events?.map((event, idx) => {
            ...
            return (
              <div key={idx} className={`flex items-center gap-4 py-2 border-b border-cyber-border-subtle last:border-0 ${past ? 'opacity-40' : ''}`}>
                ...
                <div className="flex-1 text-cyber-text-primary">
                  {event.event}
                  {past && <span className="ml-2 text-xs text-cyber-text-muted">(passed)</span>}
                </div>
                <div className={`px-2 py-1 rounded text-xs font-bold ${
                  past ? 'bg-gray-500/20 text-gray-500' :
